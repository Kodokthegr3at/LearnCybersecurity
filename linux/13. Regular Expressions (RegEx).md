...
<!-- LC-CURRICULUM-START -->
> **Curriculum ID:** `LC-020` | **Phase 1:** Linux  
> **Est. study:** 5-6h | **Level:** Intermediate  
> **Prerequisites:** LC-019  
> **Book map:** Friedl Â Mastering Regular Expressions (full); Shotts Â The Linux Command Line Ch.20
<!-- LC-CURRICULUM-END -->
...
| 7 | Security Note | ReDoS, bypass WAF & regex forensik | ReDoS, WAF bypass & forensic regex | ReDoSとWAFバイパス・正規表現フォレンジクス |
...
## 7. 🔐 Security Note — ReDoS, WAF Bypasses & Forensic RegEx Signatures

### 🇮🇩 Bahasa Indonesia
Dalam offensive dan defensive cybersecurity, Regular Expression adalah pedang bermata dua:
1. **Regular Expression Denial of Service (ReDoS)**: Ketika pola regex mengandung quantifier bertingkat (*nested quantifiers*) atau overlap alternasi (seperti `(a+)+$` atau `([a-zA-Z0-9]+)*`), regex engine berbasis NFA (*Nondeterministic Finite Automaton*) akan mengalami **Catastrophic Backtracking**. Memproses input seperti `aaaaaaaaaaaaaaaaaaaaaaaaaaaa!` membutuhkan waktu eksponensial ($O(2^n)$), membekukan CPU server dan melumpuhkan aplikasi (*Mastering Regular Expressions*, Friedl).
2. **WAF & Filter Evasion**: Input validation regex yang tidak ketat dapat dibypass:
   - Anchor newline: Pola `^https?://.*` tanpa flag multiline bisa dibypass menggunakan newline `\n`.
   - Case sensitivity: Penyerang menggunakan variasi huruf besar-kecil (`<sCrIpt>`) jika regex tidak menggunakan flag `-i` / `(?i)`.
   - Alternasi tidak lengkap: Filter blacklist kata kunci SQLi `UNION SELECT` dapat dibypass dengan inline comment `UNION/**/SELECT`.
3. **Forensic RegEx Signatures**: RegEx adalah fondasi aturan deteksi SIEM, IDS/IPS (Snort/Suricata), dan YARA rules untuk mendeteksi malware dan exfiltration.

### 🇬🇧 English
In offensive and defensive cybersecurity, Regular Expressions play a decisive role:
1. **Regular Expression Denial of Service (ReDoS)**: Patterns containing nested quantifiers or overlapping alternations (e.g., `(a+)+$` or `(x+x+)+y`) cause NFA-based regex evaluation engines to trigger **Catastrophic Backtracking**. An attacker supplying malicious non-matching input (e.g., `aaaaaaaaaaaaaaaaaaaa!`) drives CPU usage to 100%, causing service exhaustion (*Mastering Regular Expressions*, Friedl; *The Web Application Hacker's Handbook*).
2. **WAF & Input Filter Evasion**: Loose regex definitions introduce filter bypasses:
   - Multiline Anchor Bypasses: Using `^...$` instead of strict `\A...\z` allows injection via prepended/appended newlines `\n`.
   - Character Class Omissions: Insufficiently constrained character sets permit encoded payloads (`%00`, `%0a`, Unicode normalization bypasses).
   - Obfuscated Keywords: Evading `SELECT.*FROM` via comments `SELECT/*bypass*/FROM`.
3. **Forensic Threat Hunting Signatures**: Essential regex patterns power SIEM detection queries, Snort/Suricata rules, and YARA signatures to detect IOCs (Indicators of Compromise).

### 🇯🇵 日本語
サイバーセキュリティにおいて、正規表現は攻撃と防御の両面で極めて重要な技術です：
1. **正規表現サービス拒否（ReDoS）と破滅的バックトラッキング**: ネストされた量指定子（例：`(a+)+$`）や重複する選択肢を含む正規表現は、NFA型正規表現エンジンにおいて**破滅的バックトラッキング（Catastrophic Backtracking）**を引き起こします。悪意ある入力（`aaaaaaaaaaaaa!`）によって計算量が指数関数的に増大し、CPUリソースを枯渇させます（『Mastering Regular Expressions』『The Web Application Hacker's Handbook』参照）。
2. **WAF / 入力フィルターの回避**: 不完全な正規表現フィルターの迂回：
   - 改行コードによるアンカーの回避（`^` / `$` の代わりに `\A` / `\z` を使用すべき箇所）
   - 大文字小文字の混合（`(?i)` フラグの欠落）
   - インラインコメントによるシグネチャ回避（SQLi における `UNION/**/SELECT`）
3. **脅威検知・フォレンジックシグネチャ**: SIEM、IDS/IPS（Snort/Suricata）、YARAルールでIOC（侵害指標）を迅速に検出するための核となります。

```bash
# ── CYBERSECURITY REGEX PATTERNS CHEATSHEET ──────────────────

# 1. IPv4 Address Matching
grep -E -o '([0-9]{1,3}\.){3}[0-9]{1,3}' /var/log/auth.log

# 2. Extract Base64 Strings (length >= 32)
grep -E -o '[A-Za-z0-9+/]{32,}={0,2}' access.log

# 3. MD5 (32 hex) / SHA256 (64 hex) Hash Detection
grep -E -i '\b[a-f0-9]{32}\b' hashes.txt      # MD5
grep -E -i '\b[a-f0-9]{64}\b' hashes.txt      # SHA-256

# 4. Detect SQL Injection attempts in Web Server Access Logs
grep -E -i "(union.*select|select.*from|exec\(|insert.*into|drop.*table)" /var/log/apache2/access.log

# 5. Extract URLs from source text
grep -E -o 'https?://[a-zA-Z0-9./?=_-]+' input.txt
```
